...
public class GameCoopScreen implements Screen{
    ...
    private void handlePlayerGravity(){
        if(!this.worldManager.entityDoesntOverlapWorld(this.player.getHitbox()) && !player.isOnALadder){
            this.player.physicalBodyMoveY(-8);

            if(this.worldManager.entityDoesntOverlapWorld(this.player.getHitbox())){
